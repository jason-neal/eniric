...
for band in ["Z", "H", "J", "Y", "K"]:
    ...
    for sptype in ["M0", "M3", "M6", "M9"]:
        file_to_read = "Spectrum_{0}-PHOENIX-ACES_{1}band_vsini1.0_R100k_res3.txt".format(sptype, band)
        wave_stellar, flux_stellar = io.pdread_2col(os.path.join(eniric.paths["resampled"], file_to_read))
    ...
    plt.savefig(os.path.join(path, "{}_band_center_marking.png".format(band)), bbox_inches="tight")
