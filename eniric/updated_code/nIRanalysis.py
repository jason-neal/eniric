...
def calculate_ratios(spectrum):
    ...
    flux_band = np.array(flux_band)*np.array(wav_band)  # passing it to photon counts
...
def resampler(spectrum_name = "results/Spectrum_M0-PHOENIX-ACES_Jband_vsini1.0_R100k.txt", sampling=3.0, plottest=False):
    ...
    filetowrite = resampled_dir + spectrum_name[12:-4]+"_res"+str(int(sampling))+".txt" # note that this was changed with resampled_new!
